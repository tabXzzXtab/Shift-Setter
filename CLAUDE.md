...
| `npm run test:db` | Assertion suite + 95 negative controls, all rolled back |
...
| `npm run check:pdf-split` | A day taller than a page: splits at the footer, resumes under the header, loses nobody |

Account creation runs through the `create-account` Edge Function, because it
needs the service-role key. So do `update-account` and `delete-account`, for the
same reason — `auth.users` is not an exposed schema and never will be. The
DECISION in a removal is not theirs: `public.delete_account()` makes it in the
database and returns which act it performed. Deploy one with:
