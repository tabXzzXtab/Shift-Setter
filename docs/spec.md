...
**A Neka is reversible; never having been asked is not.** Öppna Pass lists every opening, including the ones this worker turned down, and every card there books through the same `accept_offer`. It takes an offer row in `offered` **or in `declined`** — `declined` is written by the Neka button and by nothing else, so what is being reversed is this person's own answer, given again. Everything else still refuses: a `withdrawn` row is the system's word rather than theirs (the pass filled, the shift was called off, Avboka released and blocked them), and a pass the tier walk never offered has no row at all. **Booking cannot step around the ranking** — förval, lateness and can't-work decide who is asked, and a worker who could book anything open would make that list advisory. Invariant 2 is unchanged underneath all of it: the slot is still one assignment per person per day, enforced by the index, and losing a race still reads *"Någon annan hann först."*
...
**A leader's own span is read-only at stage 1, and the admin corrects it at stage 2.** On Bekräfta Pass the arbetsledare sees BÖRJAR and SLUTAR on their own row as plain text; only TIMMAR is theirs to type, because lunch comes off the envelope and nobody else knows how long it was (invariant 1). A person stating when they personally were on site, on the row that pays them, is the one claim the two stages exist to check — so times are the claim somebody else verifies and hours stay the claim they make. Worker rows are untouched: their times are the pass's, the leader was the one who was there, and correcting them is the whole job of that screen.

**Granska Pass is the only screen that edits a leader's span**, and it edits `own_start`/`own_end` rather than the pass — so correcting one leader moves nobody else on the shift. `approve_day()` decides that from the row's own `source`, not from anything the caller sends: a payload naming a pass for a leader's row is routed to the row anyway, and one that omits it is routed there too. The interface is decorative (Section 6); this is the boundary. A worker's row cannot carry an own span at all — `tilldelning_own_span_is_the_leaders` refuses it — so the mistake in the other direction stops the approval dead rather than filing a worker's times where no calendar reads them.
...
4b. **An arbetsledare confirms the days they stood on, not the projects they are listed against.** If the day has an arbetsledare row, the person on that row confirms it — standing membership of the project neither grants that nor is required for it, which is what lets two leaders trade a day (Step 5d) and still have the right one answer for it. If the day has no arbetsledare row at all, there is nobody to point at and membership is enough; that is the branch the bristsurvey and a day whose leaders were all committed elsewhere fall into. This is a per-row scope, not a role check — the database must enforce it row by row. **A flagged day (Step 5c) is outside every leader's scope**: admin and only admin confirms it.
...
11. **The last active admin cannot be removed, demoted, or paused.** Admin is the only role that can change a role, so admin is the one whose disappearance is unrecoverable: otherwise nobody can promote anyone back and the system needs direct database access to recover. Leaders are not scarce in this sense — the guard counts active admins, and it does not protect the last arbetsledare.
